...
import input_data_OCR as input_data
...
IR_database = os.path.join(os.path.dirname(operationPath),'OCRdatabase')
IR_logs = os.path.join(os.path.dirname(operationPath),'IR_logs')
IR_model = os.path.join(os.path.dirname(operationPath),'IR_Models')

trainDir = os.path.join(IR_database,'TrainingData','Train\\')
validateDir = os.path.join(IR_database,'ValidateData','Validate\\')
testDir = os.path.join(IR_database,'TestData','Test\\')
...
with tf.name_scope('inputs'):
    xs = tf.placeholder(tf.float32,[None,128,128,1],name='Images') # 不规定有多少个图片’None‘，但是每一个图片都有784个点
    ys = tf.placeholder(tf.float32,[None,11],name='Labels') # 不规定有多少个输出’None‘，但是每个输出都是10个点（0-9） 
...
Act5 = layer.relu(BN5,name='Tanh5')
...
BN6 = layer.batch_norm_layer(conv6,trainphase,'BN6')
Act6 = layer.relu(BN6,name='Tanh6')
...
confusionmatrix = tf.confusion_matrix(tf.argmax(ys,1),tf.argmax(prediction,1),11,name='Confusion_matrix')
