...
import input_data_5class as input_data
...
validateDir = os.path.join(IR_database,'ValidateData','ValidateFilted10\\')
testDir = os.path.join(IR_database,'TestData','TestFilted10\\')
...
with tf.name_scope('inputs'):
    ...
    ys = tf.placeholder(tf.float32,[None,5],name='Labels') # 不规定有多少个输出’None‘，但是每个输出都是10个点（0-9） 
...
confusionmatrix = tf.confusion_matrix(tf.argmax(ys,1),tf.argmax(prediction,1),5,name='Confusion_matrix')
    ...
    try:
        for step in np.arange(MAX_STEP):
            ...
            if step % 50 == 0:
                ...
                val_loss,val_acc,val_cm = sess.run([loss,validate_acc,confusionmatrix],feed_dict={xs:val_imgs,ys:val_lbls,keep_prob:1,trainphase:False})
                ...
                summary_val = sess.run(merged,feed_dict={xs:val_imgs,ys:val_lbls,keep_prob:1,trainphase:False})
...
#            
            ...
            if  (step + 1) == MAX_STEP:                              
                ...
                for i in range(2):
                    ...
                    tst_acc,Pre,tst_cm = sess.run([test_acc,prediction,confusionmatrix],feed_dict={xs:tst_imgs,ys:tst_lbls,keep_prob:1,trainphase:False}) 
