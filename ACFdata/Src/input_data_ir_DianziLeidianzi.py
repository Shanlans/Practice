    ...
    with tf.name_scope('Get_data_labels'):
        ...
        for file in os.listdir(file_dir):
            ...
            if name[0] == 'Dianzi':
                ...
                Other.append(file_dir+file)
                label_other.append(2)
            else:
                pass
        
        print('There are %d Dianzi;\nThere are %d Zangwu;\nThere are %d Other;\n'
              %(len(Dianzi),len(Zangwu),len(Other)))
        
        image_list = np.hstack((Dianzi,Zangwu,Other))
        label_list = np.hstack((label_dianzi,label_zangwu,label_other))
        ...
        label_list=tf.one_hot(label_list,3,1,0,-1)
#        
...
def get_batch(image,label,image_W,image_H,image_Channel,batch_size,shuffle=True,number_thread=1000,capacity=1000):
    ...
    #    image = tf.image.resize_image_with_crop_or_pad(image,image_H,image_W)
        ...
        label_batch = tf.reshape(label_batch, [batch_size,3])
