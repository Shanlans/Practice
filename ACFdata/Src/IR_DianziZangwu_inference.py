...
#loadModelDir = os.path.join(IR_model,TrainingPlan,'2017-12-27-11-42','IR.ckpt-4999') # Good quality
#loadModelDir = os.path.join(IR_model,TrainingPlan,'2018-01-11-16-54','IR.ckpt-4999')
loadModelDir = os.path.join(IR_model,TrainingPlan,'2018-01-12-18-13','IR.ckpt-4000') # 2018/1/12 night
#loadModelDir = os.path.join(IR_model,TrainingPlan,'2018-01-24-18-55','IR.ckpt-4000') #train0
#loadModelDir = os.path.join(IR_model,TrainingPlan,'2018-01-25-08-46','IR.ckpt-4000') #train1
#loadModelDir = os.path.join(IR_model,TrainingPlan,'2018-01-25-13-36','IR.ckpt-3000') #train2
#loadModelDir = os.path.join(IR_model,TrainingPlan,'2018-01-25-13-36','IR.ckpt-3000') #train3
#loadModelDir = os.path.join(IR_model,TrainingPlan,'2018-01-25-14-41','IR.ckpt-4999')



workbookdir = os.path.join(testLogs,'Compare.xlsx')
workbook = xlsxwriter.Workbook(workbookdir)
...
worksheet.write('C1', 'Filename', bold)
...
conv1,w1,_,_,b1 = layer.conv_layer(inputs=xs,size_in=size_in,size_out=size_out,kernel_size=CONV1_KENEL_SIZE,device=1,name='conv1')
...
IMAGE_HEIGHT = IMAGE_HEIGHT
IMAGE_WIDTH = IMAGE_WIDTH
conv2,w2,_,_,b2 = layer.conv_layer(inputs=pool1,size_in=size_in,size_out=size_out,kernel_size=CONV2_KENEL_SIZE,device=1,name='conv2')
...
IMAGE_HEIGHT = IMAGE_HEIGHT
IMAGE_WIDTH = IMAGE_WIDTH
conv3,w3,_,_,b3 = layer.conv_layer(inputs=pool2,size_in=size_in,size_out=size_out,kernel_size=CONV3_KENEL_SIZE,device=1,name='conv3')
...
IMAGE_HEIGHT = IMAGE_HEIGHT
IMAGE_WIDTH = IMAGE_WIDTH
conv4,w4,_,_,b4 = layer.conv_layer(inputs=pool3,size_in=size_in,size_out=size_out,kernel_size=CONV4_KENEL_SIZE,device=1,name='conv4')
...
IMAGE_HEIGHT = IMAGE_HEIGHT
IMAGE_WIDTH = IMAGE_WIDTH
conv5,w5,_,_,b5 = layer.conv_layer(inputs=pool4,size_in=size_in,size_out=size_out,kernel_size=CONV5_KENEL_SIZE,device=1,name='conv5')
...
IMAGE_HEIGHT = IMAGE_HEIGHT
IMAGE_WIDTH = IMAGE_WIDTH
conv6,w6,_,_,b6 = layer.conv_layer(inputs=pool5,size_in=size_in,size_out=size_out,kernel_size=CONV6_KENEL_SIZE,device=1,name='conv6')
...
with tf.Session(config=config) as sess:
    ...
    testwriter = tf.summary.FileWriter(testLogs,sess.graph)  
...
#                saver.save(sess, checkpoint_path, global_step=step)                            
            ...
            sess.run(prediction,feed_dict={xs:tst_imgs,ys:tst_lbls,keep_prob:0.5,trainphase:False})
            elapsed_time = (time.time() - start_time)/BATCH_SIZE
#            summary_str = sess.run(merged,feed_dict={xs:tst_imgs,ys:tst_lbls,keep_prob:0.5,trainphase:False})
#            testwriter.add_summary(summary_str, step)
            ...
            if(step + 1) == MAX_STEP:
                P_dict,L_dict = sess.run([Prediction_dict,Label_dict],feed_dict={xs:tst_imgs,ys:tst_lbls,keep_prob:0.5,trainphase:False})
                
#                for index,i in enumerate(P_dict):
#                    if Pre[index][i] < 0.90:
#                        if i == 0 :
#                            pass
#                        elif i == 1 :
...
#                print([Pre[i][item] for i, item in enumerate(P_dict)])               
                row = 1 
                col = 0 
                for P in P_dict:                     
                ...
                col = 2
                for f in test:
                    worksheet.write_string(row,col,f) 
                    row += 1
