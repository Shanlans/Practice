...
layer_name = []
layerInfos = []
currentLayer = [IMAGE_WIDTH, 1, 1, 0.5]
...
logs_train_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\ACF_TrainLogs\\ACF_TAN_4CONV_GAP_BNwithRF\\'
model_train_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\ACF_Models\\ACF_TAN_4CONV_GAP_BNwithRF\\'
...
conv1,w1,conv1pad,conv1stride = layer.conv_layer_withtah(inputs=BN0,size_in=size_in,size_out=size_out,kernel_size=CONV1_KENEL_SIZE,name='conv1')
print('conv1 shape= ', conv1.get_shape())
print('conv1 pad = ',conv1pad)
convent.append([CONV1_KENEL_SIZE,conv1stride,conv1pad])
layer_name.append('conv1')
## pool1 layer ##
...
pool1,pool1pad,pool1stride = layer.max_pool_2x2(inputs=BN1,name='maxpool1')
print('pool1 shape= ', pool1.get_shape())
print('pool1 pad= ', pool1pad)
convent.append([2,pool1stride,pool1pad])
layer_name.append('pool1')
...
IMAGE_HEIGHT = IMAGE_HEIGHT
IMAGE_WIDTH = IMAGE_WIDTH
conv2,w2,conv2pad,conv2stride = layer.conv_layer_withtah(inputs=pool1,size_in=size_in,size_out=size_out,kernel_size=CONV2_KENEL_SIZE,name='conv2')
print('conv2 shape= ', conv2.get_shape())
print('conv2 pad = ',conv2pad)
convent.append([CONV2_KENEL_SIZE,conv2stride,conv2pad])
layer_name.append('conv2')
...
IMAGE_HEIGHT = IMAGE_HEIGHT/2
IMAGE_WIDTH = IMAGE_WIDTH/2
pool2,pool2pad,pool2stride = layer.max_pool_2x2(inputs=conv2,name='maxpool2')
print('pool2 shape= ', pool2.get_shape())
print('pool2 pad= ',pool2pad)
convent.append([2,pool2stride,pool2pad])
layer_name.append('pool2')
...
IMAGE_HEIGHT = IMAGE_HEIGHT
IMAGE_WIDTH = IMAGE_WIDTH
conv3,w3,conv3pad,conv3stride = layer.conv_layer_withtah(inputs=pool2,size_in=size_in,size_out=size_out,kernel_size=CONV3_KENEL_SIZE,name='conv3')
print('conv3 shape= ', conv3.get_shape())
print('conv3 pad = ',conv3pad)
convent.append([CONV3_KENEL_SIZE,conv3stride,conv3pad])
layer_name.append('conv3')
...
IMAGE_HEIGHT = IMAGE_HEIGHT/2
IMAGE_WIDTH = IMAGE_WIDTH/2
pool3,pool3pad,pool3stride = layer.max_pool_2x2(inputs=conv3,name='maxpool3')
print('pool3 shape= ', pool3.get_shape())
print('pool3 pad = ',pool3pad)
convent.append([2,pool3stride,pool3pad])
layer_name.append('pool3')
...
IMAGE_HEIGHT = IMAGE_HEIGHT
IMAGE_WIDTH = IMAGE_WIDTH
conv4,w4,conv4pad,conv4stride = layer.conv_layer_withtah(inputs=pool3,size_in=size_in,size_out=size_out,kernel_size=CONV4_KENEL_SIZE,name='conv4')
print('conv4 shape= ', conv4.get_shape())
print('conv4 pad = ',conv4pad)
convent.append([CONV4_KENEL_SIZE,conv4stride,conv4pad])
layer_name.append('conv4')
...
with tf.name_scope('GAP'):
    ...
    GAPpad = layer.padding_cal(glpooling.get_shape().as_list()[1],height,conv4.get_shape().as_list()[1],height)
    GAPstride = height
print('GAP shape= ', glpooling.get_shape())
print('GAP pad = ',GAPpad)
convent.append([height,GAPstride,GAPpad])
layer_name.append('GAP')
...
with tf.Session() as sess:
    ...
    for i in range(len(convent)):
        currentLayer = layer.outFromIn(convent[i], currentLayer)
        layerInfos.append(currentLayer)
        layer.printLayer(currentLayer, layer_name[i])
    print ("------------------------")
...
#            
            ...
            if  (step + 1) == MAX_STEP:
                ...
                image_save_path = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\Src\\ACF_TAN_4CONV_GAP_BNwithRF\\image\\'
