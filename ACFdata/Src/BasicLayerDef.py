...
def xavier_initial(inputs,size_in,size_out,kernel_size=5,conv=True,uniform=True,name="Xavier",trainphase=True):
    initial = tf.contrib.layers.xavier_initializer(uniform=uniform)
    if conv:
        weight = tf.get_variable(name,shape=[kernel_size,kernel_size,size_in,size_out],initializer=initial,trainable=trainphase)
    else:
        weight = tf.get_variable(name,shape=[size_in,size_out],initializer=initial,trainable=trainphase)
    return weight
...
def conv_layer(inputs,size_in,size_out,kernel_size,device=0,name="conv",trainphase=True):
    with tf.name_scope(name):
        weights_name= '%s_W'%name
        biases_name = '%s_B'%name
        feature_name = '%s_C'%name
        GPUdevice = '/device:GPU:%s'%device
        with tf.device(GPUdevice):
            w = xavier_initial(inputs,size_in,size_out,kernel_size=kernel_size,name=weights_name,trainphase=trainphase)
#            w = tf.Variable(tf.truncated_normal([kernel_size,kernel_size,size_in,size_out],stddev=0.01),name='Weight')
            b = tf.Variable(tf.constant(0.01,shape=[size_out]),name='Bias',trainable=trainphase)
            conv = tf.add(tf.nn.conv2d(inputs,w,strides=[1,1,1,1],padding='SAME'),b)
        pad = padding_cal(inputs.get_shape().as_list()[1],1,conv.get_shape().as_list()[1],kernel_size)
        tf.summary.histogram(weights_name,w)
        tf.summary.histogram(biases_name,b)
        tf.summary.histogram(feature_name,conv)
...
def dropout(inputs,keep_prob=1,name='dropout'):
    with tf.name_scope(name):
        dropout = tf.nn.dropout(x=inputs,keep_prob=keep_prob)
...
def batch_norm_layer(inputs, train_phase, name='BN'):
    with tf.name_scope(name):
        ...
        batch_mean = tf.reduce_sum(inputs,axises,name='batch_mean')
        batch_var = tf.reduce_mean((tf.subtract(inputs,batch_mean)**2),axises,name='batch_var')
