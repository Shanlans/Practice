...
with tf.name_scope('inputs'):
    xs = tf.placeholder(tf.float32,[None,32,32,3],name='Images') # 不规定有多少个图片’None‘，但是每一个图片都有784个点
...
#conv1_1,w1_1,_,_,_ = layer.conv_layer(inputs=xs,size_in=size_in,size_out=size_out,kernel_size=1,name='conv1_1')
#conv1_3,w1_3,_,_,_ = layer.conv_layer(inputs=xs,size_in=size_in,size_out=size_out,kernel_size=3,name='conv1_3')
#conv1_1_3 = tf.concat([xs,conv1,conv1_1,conv1_3],axis=3)
...
drop1 = layer.dropout(BN1,keep_prob=keep_prob,name='drop1')

Act1 = layer.relu(drop1,name='Tanh1')
...
#conv2_1,w2_1,_,_,_ = layer.conv_layer(inputs=pool1,size_in=size_in,size_out=size_out,kernel_size=1,name='conv2_1')
#conv2_3,w2_3,_,_,_ = layer.conv_layer(inputs=pool1,size_in=size_in,size_out=size_out,kernel_size=3,name='conv2_3')
#conv2_1_3 = tf.concat([pool1,conv2,conv2_1,conv2_3],axis=3)
print('conv2 shape= ', conv2.get_shape())



BN2 = layer.batch_norm_layer(conv2,trainphase,'BN2')
drop2 = layer.dropout(BN2,keep_prob=keep_prob,name='drop2')
Act2 = layer.relu(drop2,name='Tanh2')
...
#conv3_1,w3_1,_,_,_ = layer.conv_layer(inputs=pool2,size_in=size_in,size_out=size_out,kernel_size=1,name='conv3_1')
#conv3_3,w3_3,_,_,_ = layer.conv_layer(inputs=pool2,size_in=size_in,size_out=size_out,kernel_size=3,name='conv3_3')
#conv3_1_3 = tf.concat([pool2,conv3,conv3_1,conv3_3],axis=3)
print('conv3 shape= ', conv3.get_shape())

#
BN3 = layer.batch_norm_layer(conv3,trainphase,'BN3')
drop3 = layer.dropout(BN3,keep_prob=keep_prob,name='drop3')
Act3 = layer.relu(drop3,name='Tanh3')
...
Act4 = layer.relu(BN4,name='Tanh4')



height = 4
with tf.name_scope('GAP'):
    glpooling = tf.nn.avg_pool(Act4,ksize=[1,height,height,1],strides=[1,height,height,1],padding='VALID')
...
#            sess.run(train_step,feed_dict={xs:tra_imgs,ys:tra_lbls})
            if step % 50 == 0:
                ...
                cv1_image_group = [sess.run(i,feed_dict={xs:tra_imgs,keep_prob:0.5,trainphase:True}) for i in c1_image]               
                for i in cv1_image_group:
                    train_writer.add_summary(i,step)
                    
                cv2_image_group = [sess.run(i,feed_dict={xs:tra_imgs,keep_prob:0.5,trainphase:True}) for i in c2_image]               
                for i in cv2_image_group:
                    train_writer.add_summary(i,step)
                
                cv3_image_group = [sess.run(i,feed_dict={xs:tra_imgs,keep_prob:0.5,trainphase:True}) for i in c3_image]               
                for i in cv3_image_group:
                    train_writer.add_summary(i,step)
                    
                cv4_image_group = [sess.run(i,feed_dict={xs:tra_imgs,keep_prob:0.5,trainphase:True}) for i in c4_image]               
...
#            
            ...
            if  (step + 1) == MAX_STEP:
                cv1_save_image_group = [sess.run(i,feed_dict={xs:tra_imgs,keep_prob:0.5,trainphase:False}) for i in c1_save_image]
                cv2_save_image_group = [sess.run(i,feed_dict={xs:tra_imgs,keep_prob:0.5,trainphase:False}) for i in c2_save_image]
                cv3_save_image_group = [sess.run(i,feed_dict={xs:tra_imgs,keep_prob:0.5,trainphase:False}) for i in c3_save_image]
                cv4_save_image_group = [sess.run(i,feed_dict={xs:tra_imgs,keep_prob:0.5,trainphase:False}) for i in c4_save_image]
                image_save_path = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\Src\\IR_TAN_4CONV_GAP_2BN_dropoff_3class\\image\\'
                ...
                scipy.misc.imsave(orignal_image, tra_imgs[0].reshape((32,32,3)))
                ...
                merge_image(cv4_save_image_group,1,4,4,8,4,4,cv4_image_name)                          
