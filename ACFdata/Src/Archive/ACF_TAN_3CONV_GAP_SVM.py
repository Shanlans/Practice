...
logs_train_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\ACF_TrainLogs\\ACF_TAN_3CONV_GAP_record_input\\'
model_train_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\ACF_Models\\ACF_TAN_3CONV_GAP_record_input\\'
...
with tf.name_scope('loss'):
    w = 0.5*tf.reduce_sum(tf.square(w1)) + 0.5*tf.reduce_sum(tf.square(w2)) + 0.5*tf.reduce_sum(tf.square(w3))
    print(ys.get_shape())
    print(fc2.get_shape())
    loss = layer.svmclass(fc2,ys,w,4,BATCH_SIZE)
    loss1 = layer.svmclass(fc2,ys,w,4,VALIDATE_BATCH_SIZE)
...
#            _, tra_loss, tra_acc, Pre, Lable = sess.run([train_step,cross_entropy,train_acc,prediction,train_label_batch])
            _, tra_loss, tra_acc = sess.run([train_step,loss,train_acc],feed_dict={xs:tra_imgs,ys:tra_lbls})
...
#            
            ...
            if  (step + 1) == MAX_STEP:
                ...
                image_save_path = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\Src\\ACF_TAN_3CONV_GAP_record_input\\image\\'
