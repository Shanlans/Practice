...
### 1.Creat data ### 
#number 1 to 10 data
train_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\IR_database\\Dataset1\\'
validate_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\IR_database\\Dataset2\\'
test_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\IR_database\\Dataset3\\'
logs_train_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\IR_logs\\IR_TrainLogs\\dataset5_0\\'
model_train_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\IR_Models\\dataset5_0\\'
...
#Act1 = layer.relu(BN1,name='Tanh1')
...
Act2 = layer.relu(BN2,name='Tanh2')
...
Act3 = layer.relu(BN3,name='Tanh3')
...
confusionmatrix = tf.confusion_matrix(tf.argmax(ys,1),tf.argmax(prediction,1),3,name='Confusion_matrix')
...
#            sess.run(train_step,feed_dict={xs:tra_imgs,ys:tra_lbls})
            if step % 50 == 0:
                ...
                val_loss,val_acc = sess.run([loss,validate_acc],feed_dict={xs:val_imgs,ys:val_lbls,keep_prob:0.5,trainphase:False})
...
#            
            ...
            if  (step + 1) == MAX_STEP:
                ...
                merge_image(cv4_save_image_group,1,4,4,12,4,4,cv4_image_name)                          
                tst_imgs,tst_lbls = sess.run([test_batch, test_label_batch])
                for i in range(2):
                    start_time = time.time()
                    tst_acc,Pre,cm = sess.run([test_acc,prediction,confusionmatrix],feed_dict={xs:tst_imgs,ys:tst_lbls,keep_prob:0.5,trainphase:False}) 
