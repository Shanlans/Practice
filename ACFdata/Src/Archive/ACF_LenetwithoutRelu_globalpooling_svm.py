...
logs_train_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\ACF_TrainLogs\\'
model_train_dir = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\ACF_Models\\'
...
conv1,w1 = layer.conv_layer_withoutRelu(inputs=xs,size_in=size_in,size_out=size_out,kernel_size=CONV1_KENEL_SIZE,name='conv1')
...
IMAGE_HEIGHT = IMAGE_HEIGHT
IMAGE_WIDTH = IMAGE_WIDTH
conv2,w2 = layer.conv_layer_withoutRelu(inputs=pool1,size_in=size_in,size_out=size_out,kernel_size=CONV2_KENEL_SIZE,name='conv2')
...
#IMAGE_HEIGHT = IMAGE_HEIGHT
#IMAGE_WIDTH = IMAGE_WIDTH
#conv3,w3 = layer.conv_layer(inputs=pool2,size_in=size_in,size_out=size_out,kernel_size=CONV3_KENEL_SIZE,name='conv3')
#print('conv3 shape= ', conv3.get_shape())

#height = 7
#with tf.name_scope('GAP'):
#    glpooling = tf.nn.avg_pool(conv3,ksize=[1,height,height,1],strides=[1,height,height,1],padding='VALID')
...
size_in = FC1_KENEL_NUM
size_out= CLASS_NUM
fc2,wfc2 = layer.fc_layer_withoutRelu(inputs=drop1,size_in=size_in,size_out=size_out,name='fc2')
print('fc2 shape= ', fc2.get_shape())
...
print('prediction shape= ', fc2.get_shape())
...
loss = layer.svmclass(fc2,ys,wfc2,4,BATCH_SIZE)
loss1 = layer.svmclass(fc2,ys,wfc2,4,VALIDATE_BATCH_SIZE)
...
train_acc = evaluation(fc2,ys)
validate_acc = evaluation(fc2,ys)
test_acc = evaluation(fc2,ys)
...
#            _, tra_loss, tra_acc, Pre, Lable = sess.run([train_step,cross_entropy,train_acc,prediction,train_label_batch])
            scores,_, tra_loss, tra_acc = sess.run([fc2,train_step,loss,train_acc],feed_dict={xs:tra_imgs,ys:tra_lbls})
...
#                print(scores)
                ...
                val_loss,val_acc = sess.run([loss1,validate_acc],feed_dict={xs:val_imgs,ys:val_lbls})
...
#                cv3_image_group = [sess.run(i,feed_dict={xs:tra_imgs}) for i in c3_image]               
...
#                cv3_save_image_group = [sess.run(i,feed_dict={xs:tra_imgs}) for i in c3_save_image]
                image_save_path = 'D:\\pythonworkspace\\TensorflowTraining\\exercises\\Shen\\Practice\\ACFdata\\Src\\image\\'
                cv1_image_name = image_save_path + 'cv1_withoutrelu.png'
                cv2_image_name = image_save_path + 'cv2_withoutrelu.png'
#                cv3_image_name = image_save_path + 'cv3_withoutrelu.png'
...
#                merge_image(cv3_save_image_group,1,7,7,28,7,4,cv3_image_name)                             
                tst_imgs,tst_lbls = sess.run([test_batch, test_label_batch])
                start_time = time.time()
                tst_acc,Pre = sess.run([test_acc,fc2],feed_dict={xs:tst_imgs,ys:tst_lbls}) 
